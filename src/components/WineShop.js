...
function WineShop() {
  ...
  // 在頁面重新加載前捕捉事件並保存當前的購物車到 localStorage
  useEffect(() => {
  ...
  }, [wines.data, wines.dataLoadState]);

  useEffect(() => {
    setCurrentPage(1);
  }, [location]);

  const changedFilter = (filterText) => {
    setCurrentPage(1);
    switch (filterText) {
      case "red":
